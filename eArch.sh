...
configure_nvidia_wayland() {
    local t=$1
    info "Настройка NVIDIA для Wayland (DRM KMS)..."
...
install_desktop_kde() {
    local t=$1 variant=$2
    info "Установка KDE Plasma (${variant}, Wayland)..."
    ...
    info "Настройка SDDM на Wayland-сессию..."
...
human_desktop() {
    case $1 in
        1) echo "KDE Plasma (полная, Wayland)" ;;
        2) echo "KDE Plasma (облегчённая, Wayland)" ;;
        3) echo "GNOME (полная)" ;;
...
select_gpu() {
    ...
    c=$(ui_menu "Выберите драйверы видеокарты:" 12 60 5 \
        ...
        "4" "Nvidia open" \
        "0" "Не устанавливать драйверы") || return 1
    echo "$c"
}
...
EOF
    ...
    case ${settings_ref[GPU]} in
        ...
        3) install_gpu_nvidia /mnt dkms
           configure_nvidia_wayland /mnt ;;
        4) install_gpu_nvidia /mnt open
           configure_nvidia_wayland /mnt ;;
    esac
