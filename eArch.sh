...
# Wayland-сессия в Plasma 6 входит в пакет plasma-workspace.
# Отдельный plasma-wayland-session больше не существует.
# Для X11 нужен plasma-x11-session (опционально).
...
install_desktop_kde() {
    ...
    if [[ $variant == full ]]; then
        # Полная установка через метапакеты
        arch-chroot "$t" pacman -S --noconfirm --needed \
    ...
    # Опционально: X11-сессия (если пользователь захочет)
    # arch-chroot "$t" pacman -S --noconfirm --needed plasma-x11-session
...
show_settings_menu() {
    ...
    while true; do
        local disk_v lang_v region_v enc_v desk_v swap_v user_v kernel_v gpu_v
...
manual_install_flow() {
    ...
    while true; do
        ...
        local disk_v lang_v region_v user_v desk_v kernel_v gpu_v
