...
# ============================================================================
#  UTF-8 локаль и кириллический шрифт (ОБЯЗАТЕЛЬНО до whiptail!)
# ============================================================================
setup_utf8_locale() {
    if [[ -f /etc/locale.gen ]]; then
        grep -q '^ru_RU.UTF-8' /etc/locale.gen || \
            sed -i 's/^#\s*ru_RU.UTF-8/ru_RU.UTF-8/' /etc/locale.gen
...
setup_cyrillic_font() {
    if command -v setfont >/dev/null 2>&1; then
        ...
        setfont UniCyr_8x16 2>/dev/null || true
    fi
}
...
check_dependencies() {
    ...
    if ((${#missing[@]})); then
        warn "Отсутствуют зависимости: ${missing[*]}. Устанавливаю..."
    ...
    if ! ping -c1 -W2 archlinux.org >/dev/null 2>&1; then
        warn "Нет доступа к archlinux.org — проверьте подключение к сети."
    fi
}
...
# ------------------------------ pacman.conf --------------------------------
# Универсальная настройка: работает независимо от исходного состояния файла.
configure_pacman_common() {
    local conf=$1
    [[ -f $conf ]] || return 0
    ...
    # ParallelDownloads — всегда 25 (удаляем любую старую строку)
    sed -i '/^#\?ParallelDownloads/d' "$conf"
    sed -i '/^\[options\]/a ParallelDownloads = 25' "$conf"
}

configure_archiso_pacman() {
    configure_pacman_common /etc/pacman.conf
}

configure_pacman_target() {
    configure_pacman_common "$1/etc/pacman.conf"
}

# MAKEFLAGS для быстрой сборки AUR
configure_makepkg_target() {
    local target=$1
    local conf="$target/etc/makepkg.conf"
    [[ -f $conf ]] || return 0
    sed -i 's/^#\?MAKEFLAGS=.*/MAKEFLAGS="-j$(nproc)"/' "$conf"
}

cleanup_pacman_lock() {
    if [[ -f /var/lib/pacman/db.lck ]]; then
        if pgrep -x pacman >/dev/null; then
            warn "pacman уже работает — не удаляю db.lck"
        else
...
ui_main_menu() {
    whiptail --title "$UI_TITLE" \
             --ok-button "Выбрать" \
             --cancel-button "Выход" \
             --menu "Выберите действие:" 16 70 5 \
...
update_pacman_database() {
    info "Обновление базы данных pacman..."
    cleanup_pacman_lock
...
install_grub() {
    ...
    info "Установка загрузчика GRUB..."
    if [[ -d /sys/firmware/efi ]]; then
...
add_other_os_to_grub() {
    ...
    info "Добавление существующей ОС в загрузчик GRUB..."
    mkdir -p "$target/os"
    if [[ -n $os_subvol ]]; then
    ...
    info "Обновление конфигурации GRUB..."
    generate_grub_config "$target"
    umount "$target/os" 2>/dev/null || true
    rmdir "$target/os" 2>/dev/null || true
    ok "Существующая ОС добавлена в загрузчик"
}

# ------------------------------ AUR / yay ----------------------------------
install_yay() {
    local target=$1 user=$2
    info "Установка AUR-хелпера yay..."

    arch-chroot "$target" pacman -S --noconfirm --needed \
        git base-devel go

    local tmpdir="/tmp/yay-build"
    arch-chroot "$target" rm -rf "$tmpdir"
    arch-chroot "$target" mkdir -p "$tmpdir"
    arch-chroot "$target" chown "$user:$user" "$tmpdir"

    if ! arch-chroot "$target" runuser -u "$user" -- bash -c "
        cd '$tmpdir' &&
...
install_aur_packages() {
    local target=$1 user=$2 packages=$3
    [[ -z $packages ]] && return 0

    # yay от root не работает — используем runuser
    info "Установка AUR-пакетов: $packages"
    if arch-chroot "$target" runuser -u "$user" -- \
        yay -S --noconfirm --needed $packages 2>&1 | tee -a "$LOG_FILE"; then
...
# ------------------------------ GPU / Desktop ------------------------------
install_gpu_intel() {
    info "Установка драйверов Intel..."
...
install_gpu_nvidia() {
    local t=$1 type=$2
    info "Установка драйверов Nvidia ($type)..."
    if [[ $type == open ]]; then
...
# ------------------------------ Установка DE ------------------------------
# Wayland-сессия в Plasma 6 входит в plasma-workspace (нет отдельного пакета).
install_desktop_kde() {
    local t=$1 variant=$2
    info "Установка KDE Plasma (${variant})..."
...
install_desktop_xfce() {
    ...
    arch-chroot "$1" systemctl enable lightdm
}

install_desktop_cinnamon() {
    info "Установка Cinnamon..."
    arch-chroot "$1" pacman -S --noconfirm --needed \
...
install_desktop_lxqt() {
    ...
    arch-chroot "$1" systemctl enable sddm
}

install_desktop_i3() {
    local t=$1 user=$2
    info "Установка i3 и компонентов..."
    arch-chroot "$t" pacman -S --noconfirm --needed \
...
sort_mirrors() {
    ...
    echo -e "\033[1;36m"
    echo "╔═══════════════════════════════════════════════════════════════════════╗"
    echo "║                    eArch — Сортировка зеркал ArchLinux                ║"
    echo "╚═══════════════════════════════════════════════════════════════════════╝"
    echo -e "\033[0m"
    ...
    info "Сортировка зеркал (reflector)..."
    if reflector --latest 20 --protocol https --sort rate \
                 --save /etc/pacman.d/mirrorlist; then
        ok "Зеркала успешно отсортированы"
    else
        err "Ошибка сортировки зеркал"
        [[ -f /etc/pacman.d/mirrorlist.backup ]] && \
            cp /etc/pacman.d/mirrorlist.backup /etc/pacman.d/mirrorlist
    fi
    update_pacman_database
    ok "Сортировка зеркал завершена"
}

# ------------------------------ Человекочитаемые названия ------------------
human_disk() {
    [[ -z $1 ]] && echo "Не выбран" || echo "$1"
}
human_lang() {
    case $1 in
        ru_RU.UTF-8) echo "Русский" ;;
        uk_UA.UTF-8) echo "Українська" ;;
        be_BY.UTF-8) echo "Беларуская" ;;
        de_DE.UTF-8) echo "Deutsch" ;;
        pl_PL.UTF-8) echo "Polski" ;;
        en_US.UTF-8) echo "English" ;;
        *)           echo "Не выбран" ;;
    esac
}
human_region() {
...
human_desktop() {
    case $1 in
        1) echo "KDE Plasma (полная)" ;;
        2) echo "KDE Plasma (облегчённая)" ;;
        3) echo "GNOME (полная)" ;;
        4) echo "GNOME (облегчённая)" ;;
        5) echo "XFCE" ;;
        6) echo "Cinnamon" ;;
        7) echo "MATE" ;;
        8) echo "LXQt" ;;
...
human_user() {
    [[ -z $1 ]] && echo "Не создан" || echo "$1"
}
human_kernel() {
    case $1 in
        1) echo "Linux (обычное)" ;;
        2) echo "Linux-zen (производительное)" ;;
        3) echo "Linux-lts (стабильное)" ;;
        *) echo "Не выбрано" ;;
    esac
}
human_gpu() {
...
select_language() {
    local c
    c=$(ui_menu "Выберите язык системы:" 14 60 6 \
...
select_encryption() {
    local c
    c=$(ui_menu "Выберите тип шифрования:" 10 60 3 \
        "1" "Без шифрования" \
        "2" "LUKS1" \
...
select_swap() {
    local c
    c=$(ui_menu "Выберите размер swap:" 13 60 7 \
...
select_desktop() {
    local c
    c=$(ui_menu "Выберите рабочий стол:" 18 70 11 \
        "1" "KDE Plasma (полная)" \
        "2" "KDE Plasma (облегчённая)" \
        "3" "GNOME (полная)" \
        "4" "GNOME (облегчённая)" \
        "5" "XFCE" \
        "6" "Cinnamon" \
        ...
        "8" "LXQt" \
        "9" "i3" \
        "0" "Не устанавливать рабочий стол") || return 1
    echo "$c"
}

select_kernel() {
    local c
    c=$(ui_menu "Выберите ядро системы:" 10 60 3 \
        "1" "Linux (обычное)" \
...
select_gpu() {
    local c
    c=$(ui_menu "Выберите драйверы видеокарты:" 12 60 5 \
        "1" "Intel" \
        "2" "AMD" \
        "3" "Nvidia dkms (рекомендуемый)" \
        "4" "Nvidia open" \
        "0" "Не устанавливать (базовый mesa)") || return 1
    echo "$c"
}

create_user_interactive() {
    local user pass pass2
    user=$(ui_input "Введите имя пользователя:") || return 1
    [[ -n $user ]] || { ui_msg "Имя пользователя не может быть пустым"; return 1; }
    while true; do
        pass=$(ui_password "Введите пароль для пользователя $user:") || return 1
        pass2=$(ui_password "Подтвердите пароль:") || return 1
        if [[ -n $pass && $pass == "$pass2" ]]; then break; fi
        ui_msg "Пароли не совпадают или пустые!"
    done
    printf '%s:%s\n' "$user" "$pass"
}
...
manual_extra_packages() {
    local result
    result=$(ui_input "Введите AUR-пакеты через пробел (например: google-chrome spotify):\n\nОставьте пустым, чтобы пропустить." 12 70) || return 1
    echo "$result"
}
...
show_settings_menu() {
    ...
    while true; do
        local disk_v lang_v region_v enc_v desk_v swap_v user_v kernel_v gpu_v aur_v
        disk_v=$(human_disk     "${_out[DISK]}")
        lang_v=$(human_lang     "${_out[LANGUAGE]}")
        region_v=$(human_region "${_out[REGION]}")
        enc_v=$(human_enc       "${_out[ENCRYPTION]}")
        desk_v=$(human_desktop  "${_out[DESKTOP]}")
        swap_v=$(human_swap     "${_out[SWAP]}")
        user_v=$(human_user     "${_out[USER]}")
        kernel_v=$(human_kernel "${_out[KERNEL]}")
        gpu_v=$(human_gpu      "${_out[GPU]}")
        aur_v=$(human_aur      "${_out[AUR]}")
        ...
        case $choice in
            ...
            10) local a; a=$(manual_extra_packages) && _out[AUR]="$a" ;;
            11)
...
confirm_settings() {
    local -n _s=$1
    local msg
    msg=$(printf 'Проверьте параметры установки:\n\nДиск: %s\nЯзык: %s\nРегион: %s\nШифрование: %s\nSwap: %s\nПользователь: %s\nЯдро: %s\nРабочий стол: %s\nДрайверы GPU: %s\nAUR-пакеты: %s\n\nНачать установку?' \
        "$(human_disk     "${_s[DISK]}")" \
        "$(human_lang     "${_s[LANGUAGE]}")" \
        "$(human_region   "${_s[REGION]}")" \
        "$(human_enc      "${_s[ENCRYPTION]}")" \
        "$(human_swap     "${_s[SWAP]}")" \
        "$(human_user     "${_s[USER]}")" \
        "$(human_kernel   "${_s[KERNEL]}")" \
        "$(human_desktop  "${_s[DESKTOP]}")" \
        "$(human_gpu      "${_s[GPU]}")" \
        "$(human_aur      "${_s[AUR]:-}")")
    ui_yesno "$msg" 26 70
}
...
prepare_luks_root() {
    ...
    info "LUKS: открытие $part как cryptroot"
...
run_auto_installation() {
    ...
    echo -e "\033[1;36m"
    echo "╔═══════════════════════════════════════════════════════════════════════╗"
    echo "║                    eArch — Автоматическая установка                   ║"
    echo "╚═══════════════════════════════════════════════════════════════════════╝"
    echo -e "\033[0m"
    ...
    local disk="${settings_ref[DISK]}"
    local swap_gb="${settings_ref[SWAP]}"
    local enc="${settings_ref[ENCRYPTION]}"
    ...
    local EXISTING_FS="" EXISTING_NAME=""
    ...
    if [[ $mode == clean ]]; then
        ...
        info "Поиск свободного места на диске $disk..."
        ...
        [[ -n $free_size && $free_size -ge 10240 ]] || {
            err "Недостаточно свободного места (нужно минимум 10 ГБ)"
            return 1
        }
        info "Найдено свободное место: ${free_size}MiB с ${free_start}MiB"
        ...
        if [[ -d /sys/firmware/efi ]]; then
            ...
            if [[ -n $efi_part ]]; then
                BOOT_PART="$efi_part"
                info "Используем существующий EFI раздел: $efi_part"
            fi
        fi

        local cur=$free_start
        if [[ -z $BOOT_PART && -d /sys/firmware/efi ]]; then
            info "Создание нового EFI раздела (300MiB)..."
        ...
        if ((swap_gb > 0)); then
            info "Создание swap раздела (${swap_gb}GB)..."
        ...
        info "Создание корневого раздела btrfs..."
        ...
        info "Поиск существующих операционных систем..."
        find_existing_os "$disk" "$ROOT_PART" \
            EXISTING_ROOT_PART EXISTING_SUBVOL EXISTING_FS EXISTING_NAME || true
    fi

    if [[ -n $SWAP_PART ]]; then
        info "Активация swap на $SWAP_PART"
        mkswap "$SWAP_PART"; swapon "$SWAP_PART"
    fi

    local LUKS_UUID=""
    if [[ $enc != none ]]; then
        LUKS_UUID=$(prepare_luks_root "$ROOT_PART" "$enc" "${settings_ref[ROOTPASS]}")
        ROOT_PART="/dev/mapper/cryptroot"
    fi

    info "Форматирование корневого раздела (btrfs)"
    mkfs.btrfs -f "$ROOT_PART"

    info "Создание Btrfs подтомов и монтирование..."
    setup_btrfs_subvolumes "$ROOT_PART" "$mount_opts"

    if [[ -d /sys/firmware/efi && -n $BOOT_PART ]]; then
        info "Монтирование EFI раздела $BOOT_PART в /mnt/boot/efi"
        mkdir -p /mnt/boot/efi
        mount "$BOOT_PART" /mnt/boot/efi
    fi

    local kernel_pkgs
    case ${settings_ref[KERNEL]} in
    ...
    info "Выбрано ядро: $kernel_pkgs"

    info "Установка базовой системы (pacstrap)..."
    pacstrap /mnt base base-devel sudo $kernel_pkgs linux-firmware iucode-tool \
    ...
    info "Генерация /etc/fstab"
    genfstab -U /mnt > /mnt/etc/fstab

    info "Настройка pacman в целевой системе"
    configure_pacman_target /mnt

    info "Настройка MAKEFLAGS для сборки AUR"
    configure_makepkg_target /mnt

    info "Настройка локалей"
    configure_locales_target /mnt "${settings_ref[LANGUAGE]}"
    configure_vconsole_target /mnt "${settings_ref[LANGUAGE]}"

    info "Настройка временной зоны: ${settings_ref[REGION]}"
    arch-chroot /mnt ln -sf "/usr/share/zoneinfo/${settings_ref[REGION]}" /etc/localtime
    arch-chroot /mnt hwclock --systohc

    info "Настройка hostname и hosts"
...
EOF
    ...
    info "Создание пользователя ${settings_ref[USER]}"
    create_user /mnt "${settings_ref[USER]}" "${settings_ref[PASS]}" "${settings_ref[ROOTPASS]}"

    if [[ -n $LUKS_UUID ]]; then
        info "Настройка LUKS в mkinitcpio и GRUB"
        configure_luks_boot /mnt "$LUKS_UUID"
    fi

    case ${settings_ref[GPU]} in
        1) install_gpu_intel /mnt ;;
        2) install_gpu_amd /mnt ;;
        3) install_gpu_nvidia /mnt dkms; configure_nvidia_kms /mnt ;;
        4) install_gpu_nvidia /mnt open; configure_nvidia_kms /mnt ;;
        0|*)
            info "Драйверы GPU не выбраны — устанавливаем базовый mesa..."
            arch-chroot /mnt pacman -S --noconfirm --needed \
                mesa libgl xf86-video-vesa xf86-video-fbdev 2>/dev/null || true
            ;;
    esac

    info "Установка рабочего стола..."
    install_desktop /mnt "${settings_ref[DESKTOP]}" "${settings_ref[USER]}"

    # AUR: yay + пользовательские пакеты
    if [[ -n ${settings_ref[AUR]:-} ]]; then
        if install_yay /mnt "${settings_ref[USER]}"; then
            install_aur_packages /mnt "${settings_ref[USER]}" "${settings_ref[AUR]}" || true
        else
            warn "yay не установлен — AUR-пакеты пропущены"
        fi
    fi
...
manual_filesystem() {
    ...
    while true; do
        ...
        local part
        part=$(ui_select_kv "Выберите раздел для настройки файловой системы:" 20 90 12 "${items[@]}") || return 0
        local choice
        choice=$(ui_menu "Файловая система для $part:" 16 60 9 \
...
manual_encryption() {
    ...
    while true; do
        ...
        ((${#items[@]})) || { ui_msg "Нет разделов!"; return 1; }
        local part
        part=$(ui_select_kv "Выберите раздел для настройки шифрования:" 20 90 12 "${items[@]}") || return 0
        local choice
        choice=$(ui_menu "Шифрование для $part:" 12 60 4 \
...
manual_mountpoints() {
    local -n _mnt=$1 _fs=$2 _enc=$3
    while true; do
        local -a items=()
        while IFS= read -r line; do items+=("$line"); done < <(list_all_partitions)
        ((${#items[@]})) || { ui_msg "Нет разделов!"; return 1; }
        local part
        part=$(ui_select_kv "Выберите раздел для настройки монтирования:" 20 90 12 "${items[@]}") || return 0
        local choice
        choice=$(ui_menu "Точка монтирования для $part:" 14 60 6 \
...
manual_apply_partitioning() {
    ...
    for p in "${!_fs[@]}"; do
        ...
        if [[ $enc != none ]]; then
            info "LUKS: форматирование $p ($enc)"
            luks_format "$p" "$enc" "$MANUAL_ROOTPASS"
            local name="crypt_$(basename "$p")"
            info "LUKS: открытие $p как $name"
            luks_open "$p" "$name" "$MANUAL_ROOTPASS"
            _mapper["$p"]="/dev/mapper/$name"
            info "Форматирование /dev/mapper/$name в $fs"
            format_partition "/dev/mapper/$name" "$fs"
        else
            info "Форматирование $p в $fs"
    ...
    info "Монтирование корневого раздела $root_dev в /mnt"
    mount "$root_dev" /mnt
    mkdir -p /mnt/{boot,home,var,.snapshots}

    for p in "${!_mnt[@]}"; do
        [[ ${_mnt[$p]} == "/boot" ]] || continue
        info "Монтирование $p в /mnt/boot"
        mkdir -p /mnt/boot
    ...
    for p in "${!_mnt[@]}"; do
        ...
        info "Монтирование $p в /mnt/boot/efi"
        mkdir -p /mnt/boot/efi
...
run_manual_installation() {
    ...
    info "Применение настроек разделов..."
    ...
    local disk_type mount_opts
    disk_type=$(check_disk_type "${settings_ref[DISK]}")
    mount_opts=$(mount_options_for "$disk_type")

    if [[ $root_fs == btrfs ]]; then
        info "Настройка Btrfs подтомов..."
        umount -R /mnt 2>/dev/null || true
...
EOF
    ...
    install_grub /mnt "${settings_ref[DISK]}"
...
manual_install_flow() {
    ...
    s[KERNEL]="1"; s[DESKTOP]="0"; s[GPU]="0"; s[AUR]=""
    ...
    while true; do
        ...
        local disk_v lang_v region_v user_v desk_v kernel_v gpu_v aur_v
        disk_v=$(human_disk     "${s[DISK]}")
        lang_v=$(human_lang     "${s[LANGUAGE]}")
        region_v=$(human_region "${s[REGION]}")
        user_v=$(human_user     "${s[USER]}")
        desk_v=$(human_desktop  "${s[DESKTOP]}")
        kernel_v=$(human_kernel "${s[KERNEL]}")
        gpu_v=$(human_gpu      "${s[GPU]}")
        aur_v=$(human_aur      "${s[AUR]}")

        local choice
        choice=$(ui_menu "\n\n\n" 28 85 14 \
            "1"  "Диск ($disk_v)" \
            "2"  "Разметка диска (cfdisk)" \
            "3"  "Файловая система ($fs_count разделов)" \
            "4"  "Шифрование" \
            "5"  "Монтирование ($mnt_count разделов)" \
            "6"  "Язык ($lang_v)" \
            "7"  "Регион ($region_v)" \
            "8"  "Пользователь ($user_v)" \
            "9"  "Рабочий стол ($desk_v)" \
            "10" "Ядро ($kernel_v)" \
            "11" "Драйверы GPU ($gpu_v)" \
            "12" "AUR-пакеты ($aur_v)" \
            "13" "Начать установку") || return 0
        ...
        case $choice in
            ...
            5) manual_mountpoints PARTITION_MOUNT PARTITION_FS PARTITION_ENCRYPTION ;;
            ...
            12) local a; a=$(manual_extra_packages) && s[AUR]="$a" ;;
            13)
                if [[ -z ${s[DISK]} || -z ${s[LANGUAGE]} || -z ${s[REGION]} || -z ${s[USER]} ]]; then
                    ui_msg "Заполните обязательные параметры:\n\n- Диск\n- Язык\n- Регион\n- Пользователь"
                    continue
                fi
...
main() {
    ...
    while true; do
        local choice
        choice=$(ui_main_menu) || { ui_msg "Выход из установщика eArch."; exit 0; }
        case $choice in
            1) auto_install_flow ;;
