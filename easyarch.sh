#!/bin/bash
#  based on EasyArch by LinuxShef 
#  eArch - Установщик ArchLinux
...
configure_pacman() {
    ...
    sed -i 's/#ParallelDownloads = 5/ParallelDownloads = 25/' "$target/etc/pacman.conf"
...
show_setup_process() {
    ...
    while [ -f "$SETUP_LOG" ]; do
        if [ -f "$SETUP_LOG" ]; then
            ...
            echo "$PROGRESS" | whiptail --title "eArch Установщик" \
...
select_install_type() {
    while true; do
        CHOICE=$(whiptail --title "eArch Установщик" \
        ...
        if [ $? -ne 0 ]; then
            whiptail --title "Выход" --msgbox "Выход из установщика eArch." 8 40
            exit 0
        fi
...
show_settings_menu() {
    ...
    while true; do
        ...
        MENU_CHOICE=$(whiptail --title "eArch Установщик - Настройки" \
                               --menu "\n\n\n" 20 80 10 \
                               "1" "Диск (${DISK_VALUE})" \
                               "2" "Язык (${LANG_VALUE})" \
...
show_manual_menu() {
    ...
    while true; do
        ...
        MENU_CHOICE=$(whiptail --title "eArch Установщик - Ручная установка" \
                               --menu "\n\n\n" 24 80 13 \
                               "1" "Диск (${DISK_VALUE})" \
...
run_manual_installation() {
    ...
    echo -e "\033[1;36m"
    echo "╔═══════════════════════════════════════════════════════════════════════════════╗"
    echo "║                           eArch Установщик ArchLinux                       ║"
    echo "╚═══════════════════════════════════════════════════════════════════════════════╝"
    echo -e "\033[0m"
    echo ""
